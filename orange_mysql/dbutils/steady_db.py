...
基于任意符合DB-API 2的数据库接口模块实现与数据库的稳定连接。
...
当连接关闭、数据库连接丢失或使用频率超过可选的使用限制时，这些连接会透明地重新打开。
...
当由于连接丢失而无法执行数据库操作时，数据库游标也会透明地重新打开。

Only if the connection is lost
...
只有在执行后连接丢失的情况下，当已经从数据库中提取行时，
才会出现错误，并且光标不会自动重新打开，因为在这种情况下没有可靠的方法来恢复光标的状态。

Connections which have been marked as being in a transaction
with a begin() call will not be silently replaced either.
已标记为使用begin()调用处于事务中的连接也不会被静默替换。

A typical situation where database connections are lost
is when the database server or an intervening firewall is
shutdown and restarted for maintenance reasons.
数据库连接丢失的典型情况是，由于维护原因，数据库服务器或中间防火墙关闭并重新启动。

In such a case, all database connections would become unusable, even
...
这种方法产生了一个稳定的数据库连接，PooledDB或PersistentDB可以使用该连接来创建到线程环境中的数据库的池连接或持久连接，
例如“WebwareforPython”的应用程序服务器

Note, however, that the connections themselves may not be
thread-safe (depending on the used DB-API module).
但是，请注意，连接本身可能不是线程安全的（取决于所使用的DB-API模块）。
